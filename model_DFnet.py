...
class Net(nn.Module):
    def __init__(self, angRes):
        ...
        self.conv = nn.Conv2d(channel*4, channel*2, 1, 1, 0)   
        self.upsample = LF_Decoding(self.angRes,channel*2)

    def forward(self, x, h_ori, w_ori, scale):
        x_upscale = interpolate(x, self.angRes, size=(h_ori,w_ori), mode='bicubic')
        ...
        buffer_total = combine_center(buffer_sv,buffer_cv)  # b × 25 × c × h × w
        buffer_total = buffer_total.contiguous().view(b*(n+1),-1,h,w)

        fea = self.conv(buffer_total)
        out = self.upsample(fea,h_ori,w_ori,scale)
        out = out + x_upscale
        return out
