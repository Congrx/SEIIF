...
class Net(nn.Module):
    def __init__(self, angRes):
        ...
        self.shrink_channel = 64
        self.conv = nn.Conv2d(channel*4, self.shrink_channel, 1, 1, 0)
        self.upsample = LF_Decoding(self.angResRes,self.shrink_channel) 

    def forward(self, x, h_ori, w_ori, scale):
        x_multi = LFsplit(x, self.angResRes)
        ...
        x_upscale = x_upscale.unsqueeze(1).contiguous().view(b, -1, c, HH, WW)
        ...
        intra_fea = self.FBM(intra_fea)     # batch × an2 × C × h × w
        
        intra_fea = intra_fea.contiguous().view(b*n, -1, h, w)
        fea = self.conv(intra_fea)      # batch*an2 * C * h * w
...
if __name__ == "__main__":
    ...
    flops, params = profile(net, inputs=(input,32*2,32*2,2))
