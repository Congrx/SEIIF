...
class Net(nn.Module):
    def __init__(self, angRes):
        ...
        iter_path = 2
        total_depth = 4 * iter_path
        dpr = [x.item() for x in torch.linspace(0, 0.1, total_depth)]  
        for i in range(0,4):
            dpr_iter = dpr[i*iter_path:(i+1)*iter_path]
            self.blocks.append(MixTransfomerBlock(self.channels,4,4,4,4,2,ang_sr_ratio=1,spa_sr_ratio=2,
                                                qkv_bias=True, qk_scale=None, drop=0.0, attn_drop=0.0,drop_path=dpr_iter, act_layer=nn.GELU, norm_layer=nn.LayerNorm))
        # step3: Hierarchical Feature Aggregation & Upsampling
        self.mla = MLA(self.channels)
        self.conv = nn.Conv2d(self.channels*2, self.channels, 1, 1, 0)
        self.upsample = LF_Decoding(self.angRes,self.channels)
    ...
    def forward(self, lr, h_ori, w_ori, scale):      
        batch , _ , _ , _  = lr.size() 
        lr_upscale = interpolate(lr, self.angRes, size=(h_ori,w_ori), mode='bicubic')   
        ...
        # step3: Hierarchical Feature Aggregation & Upsampling
        fusion_feature = self.mla(hierarchical_feature[0],hierarchical_feature[1],hierarchical_feature[2],hierarchical_feature[3])
        fea = self.conv(fusion_feature)      # batch*an2 * C * h * w
        out = self.upsample(fea,h_ori,w_ori,scale) + lr_upscale
        return out
        
        
def interpolate(x, angRes, size, mode):    
    [B, _, H, W] = x.size()
    h = H // angRes         
    w = W // angRes     
    x_upscale = x.view(B, 1, angRes, h, angRes, w) 
    x_upscale = x_upscale.permute(0, 2, 4, 1, 3, 5).contiguous().view(B * angRes ** 2, 1, h, w)  
    x_upscale = F.interpolate(x_upscale, size=(size[0],size[1]), mode=mode, align_corners=False)  
    x_upscale = x_upscale.view(B, angRes, angRes, 1, size[0], size[1])           
    x_upscale = x_upscale.permute(0, 3, 1, 4, 2, 5).contiguous().view(B, 1, angRes*size[0], angRes*size[1]) 
