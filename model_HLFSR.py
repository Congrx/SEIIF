...
class Net(nn.Module):
    ...
    def forward(self, x, h_ori, w_ori, scale):		
        ...
        x_upscale = x_upscale.unsqueeze(1).contiguous().view(b, -1, c, h_ori, w_ori)      # batch * 25 * 1 * HH * WW
        ...
        x_out = LFsplit(x_out,self.angRes)     
        x_out = x_out.contiguous().view(b*n,-1,h,w)
        x_out = self.upsample(x_out,h_ori,w_ori,scale) + FormOutput(x_upscale)
